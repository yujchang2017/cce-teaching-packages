...
每一筆提交先看「提交類型」，再依類型完成檢查：

- `A：改編＋試教`：需同時檢查改編內容與試教紀錄
- `B：改編`：檢查改編說明、改編檔案或可公開內容
- `C：試教`：檢查試教情境、教學省思、照片或學生作品授權

基本檢查項目：

1. 身分對照：以回饋表單的 Google Email 到「貢獻者基本資料表」確認老師姓名、學校、縣市與任教年段。若查不到，狀態先填 `needs-revision`，請投稿者補填基本資料。
2. 內容完整：確認教案編號、提交類型、回饋或試教內容不是空白，且能判斷投稿者實際使用或改編了哪份教案。
3. 授權確認：照片、學生作品、改編檔案若要公開，必須有明確授權；沒有授權或授權不清楚時，只能標為僅供內部審核或請投稿者補件。
4. 個資檢查：公開素材不得含學生姓名、清楚可辨識臉部、聯絡方式、班級座號等未授權個資。若有疑慮，先填 `needs-revision`。
...
| 欄位 | 填寫方式 |
| --- | --- |
| `狀態` | `pending`、`approved`、`needs-revision`、`rejected` |
| `審核備註` | 給投稿者或維護者看的具體說明 |
| `審查委員` | 審查員姓名或 Email |
| `審查日期` | 完成審查日期 |
...
- `approved`：內容完整、授權清楚、沒有個資疑慮，可進入整理與公開流程。
- `needs-revision`：缺基本資料、授權不清、附件無法開啟、內容不足或需去識別化，請投稿者補件。
- `rejected`：內容與教案無關、授權不允許使用、含無法處理的個資或明顯不適合公開。
- `pending`：尚未審查或等待其他審查員確認。
