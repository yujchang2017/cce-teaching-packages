# 1.1-III 天氣、氣候與氣候變遷

> UNESCO CCE Theme 1 · Topic 1 · Level III（國小中高年級，9–12 歲）
>
> **主辦：** 教育部資訊及科技教育司　|　**執行：** 臺北市立大學　|　**計畫：** 115 年中小學氣候變遷教育推動計畫

---

## Key Idea

**全球平均氣溫正在上升，極端天氣事件愈來愈頻繁。**

臺灣近 110 年升溫約 1.6°C，是全球平均的 2 倍；2024 年全球均溫較前工業化基準高出約 1.47°C，創下史上最熱紀錄。本教案帶領學生從「天氣 vs 氣候」的基本概念出發，認識臺灣與全球的氣候變遷證據，連結情緒感受，並轉化為家庭與社區的具體行動。

## 適用資訊

| 項目 | 內容 |
|---|---|
| 年段 | 國小 3–6 年級（9–12 歲） |
| 節數 | 4 節 × 40 分鐘 = 160 分鐘 |
| 授課語言 | 繁體中文（關鍵詞附英文：weather / climate / global warming） |
| UNESCO Theme | 1. Planetary integrity and interdependence of all life |
| UNESCO Topic | 1.1 Weather, climate and climate change |

## 教學目標（對應 UNESCO 三領域）

### 認知層（Cognitive）· 5 項
- **C1** 說明氣溫在長時間尺度上的變化，以及工業革命後變化速率加快
- **C2** 調查並描述臺灣地區的氣溫範圍
- **C3** 解釋「全球暖化 / 全球增溫」一詞的意義
- **C4** 說明季節氣溫雖年年有差異，但整體趨勢上升
- **C5** 舉例全球暖化的全球與在地影響

### 社會情緒層（Social & Emotional）· 2 項
- **S1** 表達對採取行動的感受（希望、被賦能、想做出改變）
- **S2** 參與能將正向情緒與氣候行動連結的活動

### 行為層（Behavioural）· 1 項
- **B1** 與家人／在地社群合作預防、減緩或調適氣候風險

合計 **8 項學習目標**。

## 檔案清單

| 檔案 | 說明 |
|---|---|
| [`lesson_plan.md`](lesson_plan.md) | 4 節教案全文（含師生互動、時間分配、對應 LO） |
| [`ppt_script.md`](ppt_script.md) | 投影片講述稿，逐張對應教案流程 |
| [`lesson_plan.html`](lesson_plan.html) | 互動式教案全文，可在瀏覽器直接檢視 |
| [`ppt.html`](ppt.html) | 投影片簡報，可在瀏覽器直接呈現 |
| [`worksheet.html`](worksheet.html) | 互動學習單，可離線用瀏覽器開啟 |
| [`data_card.json`](data_card.json) | 關鍵數據卡片（含 TCCIP、NASA 等來源） |
| [`resources.md`](resources.md) | 延伸閱讀、影片、MOE 教案連結清單 |
| [`qa_report.md`](qa_report.md) | 品質審查報告（資料正確性 / 教學適齡性檢核） |

## 快速操作

- **教案線上預覽**：[lesson_plan.html（GitHub Pages）](https://yujchang2017.github.io/cce-teaching-packages/packages/level-iii/1.1-III/lesson_plan.html)
- **簡報線上預覽**：[ppt.html（GitHub Pages）](https://yujchang2017.github.io/cce-teaching-packages/packages/level-iii/1.1-III/ppt.html)
- **線上互動預覽**（推薦）：[worksheet.html（GitHub Pages）](https://yujchang2017.github.io/cce-teaching-packages/packages/level-iii/1.1-III/worksheet.html) — 點開即可直接與學習單互動（拖放、答題、回饋動畫皆可用）
- **下載整包**：點 [此連結](https://downgit.github.io/#/home?url=https://github.com/yujchang2017/cce-teaching-packages/tree/main/packages/level-iii/1.1-III) 用 DownGit 打包下載
- **離線使用**：將 `worksheet.html` 下載後雙擊用瀏覽器開啟即可（需連網載入 Tailwind CDN）
- **改編此教案**：Fork 本倉庫，或參閱 [CONTRIBUTING](../../../CONTRIBUTING.md)

> 註：若舊版說明指向 `htmlpreview.github.io/?...` 或 `raw.githubusercontent.com/...`，請改用上方 GitHub Pages 連結 —— htmlpreview 會把頁面放進 sandbox iframe，Tailwind JIT 與 drag/drop 事件在該情境下無法正常運作。

## 改編歷程（Fork Tree）

```
原作 · 115 年中小學氣候變遷教育推動計畫團隊 · 2026-04-16 · v1.0.0
└── （尚無衍生版本）
```

---

**授權**：內容採 [CC BY-SA 4.0](../../../CONTENT-LICENSE.md)
**版本**：1.0.0（2026-04-16 首版）
